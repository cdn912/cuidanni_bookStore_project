...
function changeBtn(){
    ...
    $(".shoppingsum .checkall").on("click","input",function(){
        ...
        if(!this.checked){
            $(".shoppingsum .msg span").html(0);
            $(".shoppingsum .sum").html(`<span>总计(不含运费)：</span><span>￥0.00</span>`);
        }else{
            render();
        }
    })
    ...
    $('.shoppingbag .shoppinglist').on('click',".select input",function(){
        ...
        for(var i = 0 ; i < inputs.length ; i++){
            ...
            if(!inputs[i].checked){
                ...
                count += Number($(this).parent().parent().parent().children(".goods").children(".amount").find("input").eq(i).val());
                total += Number($(this).parent().parent().parent().children(".goods").children(".perprice").find("span").eq(i).html())*Number($(this).parent().parent().parent().children(".goods").children(".amount").find("input").eq(i).val());
            };
        }
        $(".shoppingsum .msg span").html(count);
        $(".shoppingsum .sum").html(`<span>总计(不含运费)：</span>
        <span>￥${total}</span>`);
...
//点击结算按钮，前往支付页面
$(".shoppingsum .pay").on("click",function(){
